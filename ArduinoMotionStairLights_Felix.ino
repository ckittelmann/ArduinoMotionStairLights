...
#define LEDS_PER_STAIR 12 //12       // Number of Leds per stair.  Not yet currenlty changable - just noteable
#define BRIGHTNESS 100          // 0...255  ( used in fade7 )
#define PIN_LED 5               // LED Data pin
#define PIN_PIR_DOWN 3          // PIR Downstairs Pin
...
long offInterval = 1000;       // 1000mills * 1sec
long effectInterval = 1;//40
...
int i = 0;
int x = 0;
uint8_t topBrightness = 120;  // May preserve LED life if not running at full brightness 255?
uint8_t rnd = 0;
...
void setup() {
  ...
  pinMode(PIN_PIR_DOWN, INPUT); //3
  pinMode(PIN_PIR_UP, INPUT);  //4
  pinMode(13, OUTPUT);
...
void chooseEffects(){
  randomSeed(millis());
  r = random8(176, 177); //vorher stand 255 statt 100
...
void setPalette(){
   ...
   * red to blue
   */
  uint8_t r = random8(1, 100); // call it once first. vorher 255 statt 100
...
void walk() {
  ...
  if ( stage == stage_init ){
    ...
    gStair=0;
    gBright=0;
    effectInterval=1;//5 Einschaltzeit h̦here Zahlen = langsamer
    i = 0;
    x = 0;
    r = 0;
    g = 0;
    ...
    effectInterval = 1;//8 Ausschaltzeit h̦here Zahlen = langsamer
    gBright = 0;
    gStair = 0;
    stage = stage_dim;
